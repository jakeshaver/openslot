...
describe('POST /api/offers/:offerId/book', () => {
  ...
  it('returns 409 for already-claimed slot', async () => {
    ...
    // Pre-claim slot 0
    await store.claimSlot(offer.id, 0, { name: 'Someone', email: 'someone@test.com' });
...
describe('Offer store', () => {
  it('creates offer with correct slot count', async () => {
    const offer = await store.createOffer({
      ownerEmail: 'test@example.com',
      windows: [
  ...
  it('creates correct slot count for 45-min duration', async () => {
    const offer = await store.createOffer({
      ownerEmail: 'test@example.com',
      windows: [
  ...
  it('creates correct slot count for 60-min duration', async () => {
    const offer = await store.createOffer({
      ownerEmail: 'test@example.com',
      windows: [
  ...
  it('claims slot and updates status', async () => {
    ...
    await store.claimSlot(offer.id, 0, { name: 'Bob', email: 'bob@test.com' });
  ...
  it('marks offer as claimed when all slots are claimed', async () => {
    const offer = await store.createOffer({
      ownerEmail: 'test@example.com',
      windows: [{ start: '2026-03-10T14:00:00Z', end: '2026-03-10T15:00:00Z' }],
      duration: 30,
    ...
    await store.claimSlot(offer.id, 0, { name: 'A', email: 'a@test.com' });
    await store.claimSlot(offer.id, 1, { name: 'B', email: 'b@test.com' });
    const updated = await store.getOffer(offer.id);
    expect(updated.status).toBe('claimed');
  });

  it('auto-expires stale offers on read', async () => {
    const offer = await store.createOffer({
      ownerEmail: 'test@example.com',
      windows: [{ start: '2026-03-10T14:00:00Z', end: '2026-03-10T15:00:00Z' }],
      duration: 30,
    ...
    const read = await store.getOffer(offer.id);
